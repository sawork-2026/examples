...
class HttpServer:
    ...
    def _handle(self, conn: socket.socket):
        try:
            ...
            session_id, session, is_new_session = self.session_manager.get_or_create(cookie_session_id)
            request = request.with_session(session)
            status, headers, resp_body = self._dispatch(request)
            if is_new_session:
                headers["Set-Cookie"] = f"SESSIONID={session_id}; Path=/; HttpOnly"
    ...
    def _parse(self, data: bytes) -> Request:
        ...
        return Request(method, path, query_string, headers, body, cookies=cookies)

    def _parse_cookies(self, header: str) -> dict:
        cookies = {}
        if not header:
            return cookies
    ...
    def _send(self, conn: socket.socket, status: int, headers: dict, body: bytes):
        status_text = {200: "OK", 201: "Created", 302: "Found",
